...
export class QueueService {
	...
	private eventHandlers: Map<string, ((event: DomainEvent) => void)[]> =
		new Map();
	...
	async markProcessingStarted(taskId: string): Promise<void> {
		try {
			...
			logger.error(
				"Failed to mark processing started",
				error instanceof Error ? error : new Error(String(error)),
				{ taskId }
			);
	...
	async markProcessingCompleted(taskId: string): Promise<void> {
		try {
			...
			logger.error(
				"Failed to mark processing completed",
				error instanceof Error ? error : new Error(String(error)),
				{ taskId }
			);
			throw new Error("Failed to update task status");
	...
	async markProcessingFailed(
		...
		try {
			await this.db.queueRepository.updateStatus(
				taskId,
				"FAILED",
				errorMessage
			);

			logger.error("Task processing failed", undefined, {
				taskId,
				errorMessage,
			});
		} catch (error) {
			logger.error(
				"Failed to mark processing failed",
				error instanceof Error ? error : new Error(String(error)),
				{ taskId }
			);
			throw new Error("Failed to update task status");
	...
	async canProcessMore(): Promise<boolean> {
		try {
			...
			logger.error(
				"Failed to check processing capacity",
				error instanceof Error ? error : new Error(String(error))
			);
			return false;
		}
	...
	async getCurrentProcessingCount(): Promise<number> {
		try {
			...
			logger.error(
				"Failed to get current processing count",
				error instanceof Error ? error : new Error(String(error))
			);
			return 0;
		}
	...
	async getQueueDepthByPriority(): Promise<Record<QueuePriority, number>> {
		try {
			...
			logger.error(
				"Failed to get queue depth by priority",
				error instanceof Error ? error : new Error(String(error))
			);
	...
	async getAverageProcessingTime(): Promise<number> {
		try {
			...
			logger.error(
				"Failed to get average processing time",
				error instanceof Error ? error : new Error(String(error))
			);
