...
export interface TaskManager {
	createTask(): string;
	setTaskStatus(taskId: string, status: TaskResultResponse): void;
...
export class RedisTaskManager implements TaskManager {
	...
	async getTaskStatus(taskId: string): Promise<TaskResultResponse | null> {
		...
		try {
			...
			logger.debug("Retrieved Redis task status", {
				taskId,
				state: status.state,
			});
			return status;
			...
			logger.error(
				"Failed to parse task status from Redis",
				error instanceof Error ? error : new Error(String(error)),
				{ taskId }
			);
			return null;
