...
export class DatabaseService {
  ...
  private async cleanupStaleFiles(): Promise<void> {
    ...
    try {
      ...
      logger.warn("Could not clean up stale files", error instanceof Error ? error : new Error(String(error)));
    }
  }
  ...
  private async recoverFromCorruption(): Promise<void> {
    ...
    try {
      ...
      logger.error("Database recovery failed", error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
        ...
        while (retries > 0) {
          try {
            ...
            if (retries === 0) {
              ...
              logger.warn("Database initialization failed after all retries, attempting recovery...");
              ...
              try {
                ...
                logger.error("Database recovery failed, falling back to in-memory database");
                this.db = new PGlite(); // Fallback to in-memory
                this.dataDir = ":memory:";
                break;
              }
            }
            logger.warn(`Database initialization failed, retrying... (${retries} retries left)`);
    ...
    try {
      ...
      while (retries > 0) {
        try {
          ...
          logger.warn(`Database health check failed, retrying... (${retries} retries left)`);
...
class AudioRepositoryImpl implements AudioRepository {
  ...
  async save(fingerprint: AudioFingerprint): Promise<AudioFingerprint> {
    ...
    if (fingerprint.id) {
      ...
      return this.mapToAudioFingerprint(result.rows[0]);
    } else {
      // Insert new
      const result = await db.query(`
  ...
  async findByContentHash(hash: string): Promise<AudioFingerprint | null> {
    ...
    return result.rows.length > 0 ? this.mapToAudioFingerprint(result.rows[0]) : null;
  }
  ...
  async findByPerceptualHash(hash: string): Promise<AudioFingerprint[]> {
    ...
    return result.rows.map((row: any) => this.mapToAudioFingerprint(row));
  }
  ...
  private mapToAudioFingerprint(row: any): AudioFingerprint {
...
class CacheRepositoryImpl implements CacheRepository {
  ...
  async save(cache: AnalysisCache): Promise<AnalysisCache> {
    ...
    if (cache.id) {
      ...
      return this.mapToAnalysisCache(result.rows[0]);
    } else {
      // Insert new
      const result = await db.query(`
  ...
  async findByFingerprint(fingerprintId: string): Promise<AnalysisCache | null> {
    const db = this.getDb();
    const result = await db.query(
    ...
    return result.rows.length > 0 ? this.mapToAnalysisCache(result.rows[0]) : null;
  }
  ...
  private mapToAnalysisCache(row: any): AnalysisCache {
    return new AnalysisCache({
      id: row.id,
      audioFingerprintId: row.audio_fingerprint_id,
      bpm: parseFloat(row.bpm),
      beatTimestamps: typeof row.beat_timestamps === 'string' ? JSON.parse(row.beat_timestamps) : row.beat_timestamps,
      melodyMap: typeof row.melody_map === 'string' ? JSON.parse(row.melody_map) : row.melody_map,
      analysisInfo: typeof row.analysis_info === 'string' ? JSON.parse(row.analysis_info) : row.analysis_info,
...
class QueueRepositoryImpl implements QueueRepository {
  ...
  async dequeue(limit: number = 1): Promise<QueueItem[]> {
    ...
    return result.rows.map((row: any) => this.mapToQueueItem(row));
  }
  ...
  async findByTaskId(taskId: string): Promise<QueueItem | null> {
    ...
    return result.rows.length > 0 ? this.mapToQueueItem(result.rows[0]) : null;
  }

  async updateStatus(taskId: string, status: QueueStatus, errorMessage?: string): Promise<void> {
    const db = this.getDb();
  ...
  async getQueueStats(): Promise<QueueStatistics> {
    ...
    return new QueueStatistics(
      parseInt((queuedResult.rows[0] as any).count || "0"),
      parseInt((processingResult.rows[0] as any).count || "0"),
      parseFloat((avgProcessingResult.rows[0] as any).avg_time || "30000"), // Default 30 seconds
      parseFloat((avgWaitResult.rows[0] as any).avg_wait || "10000"), // Default 10 seconds
  ...
  async getQueuePosition(taskId: string): Promise<QueuePosition | null> {
    ...
    const position = parseInt((result.rows[0] as any).position);
