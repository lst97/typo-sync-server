...
import { AudioFingerprint, AnalysisCache, CacheHitEvent } from "../types/domain.ts";
...
export class CacheService {
  ...
  private async initializeRedis(): Promise<void> {
    if (this.options.redisUrl) {
      try {
        this.l2Redis = await connectRedis({
          hostname: new URL(this.options.redisUrl).hostname,
          port: parseInt(new URL(this.options.redisUrl).port) || 6379,
          db: parseInt(new URL(this.options.redisUrl).pathname.slice(1)) || 0,
        });
  ...
  async get(audioHash: string): Promise<AnalysisResult | null> {
    try {
      ...
      if (l1Result && !this.isValidAnalysisResult(l1Result)) {
        logger.warn("Invalid L1 cache result, removing from cache", undefined, { audioHash: audioHash.substring(0, 8) + "..." });
      ...
      if (l2Result && !this.isValidAnalysisResult(l2Result)) {
        logger.warn("Invalid L2 cache result, removing from cache", undefined, { audioHash: audioHash.substring(0, 8) + "..." });
        if (this.l2Redis) {
      ...
      if (l3Result && !this.isValidAnalysisResult(l3Result)) {
        logger.warn("Invalid L3 cache result, cache may be corrupted", undefined, { audioHash: audioHash.substring(0, 8) + "..." });
      ...
      logger.error("Cache get failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
      return null;
    }
  }
  ...
  async set(audioHash: string, result: AnalysisResult): Promise<void> {
    try {
      ...
      if (!this.isValidAnalysisResult(result)) {
        logger.error("Attempted to cache invalid analysis result", new Error("Invalid analysis result"), { 
      ...
      logger.debug("Cache set successful", { audioHash: audioHash.substring(0, 8) + "..." });
    } catch (error) {
      logger.error("Cache set failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
    }
  }
  ...
  private async setL1(audioHash: string, result: AnalysisResult): Promise<void> {
  ...
  private async getL2(audioHash: string): Promise<AnalysisResult | null> {
    ...
    try {
      ...
      logger.error("L2 cache get failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
      return null;
    }
  }
  ...
  async setL2(audioHash: string, result: AnalysisResult): Promise<void> {
    ...
    try {
      ...
      logger.error("L2 cache set failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
    }
  }
  ...
  private async getL3(audioHash: string): Promise<AnalysisResult | null> {
    try {
      ...
      const cache = await this.db.cacheRepository.findByFingerprint(fingerprint.id!);
      ...
      logger.error("L3 cache get failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
      return null;
    }
  }

  async setL3(audioFingerprintId: string, result: AnalysisResult): Promise<void> {
    try {
      // Validate the result has required fields
      if (!this.isValidAnalysisResult(result)) {
        logger.error("Invalid analysis result for L3 cache", new Error("Invalid analysis result for L3 cache"), { 
          audioFingerprintId, 
          result: {
            bpm: result.bpm,
            beat_timestamps_length: result.beat_timestamps?.length,
      ...
      logger.error("L3 cache set failed", error instanceof Error ? error : new Error(String(error)), { audioFingerprintId });
    }
  }
  ...
  async invalidate(audioHash: string): Promise<void> {
    try {
      ...
      logger.error("Cache invalidation failed", error instanceof Error ? error : new Error(String(error)), { audioHash });
    }
  }
  ...
  async clearL2Cache(): Promise<void> {
    if (this.l2Redis) {
      try {
        ...
        logger.error("L2 cache clear failed", error instanceof Error ? error : new Error(String(error)));
      }
    }
  }
  ...
  async warmCache(): Promise<void> {
    try {
      ...
      const recentCaches = await this.db.cacheRepository.cleanupExpired(cutoffDate);
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      const cutoffDate = new Date(Date.now() - this.options.l3TtlMs);
      const deletedCount = await this.db.cacheRepository.cleanupExpired(cutoffDate);
    ...
    try {
      ...
      if (this.l2Redis) {
        try {
          ...
          logger.error("Failed to get L2 memory usage", error instanceof Error ? error : new Error(String(error)));
        }
      }
  ...
  private isValidAnalysisResult(result: AnalysisResult): boolean {
    ...
    if (typeof result.bpm !== 'number' || result.bpm <= 0 || !isFinite(result.bpm)) {
      return false;
    }

    // Beat timestamps must be an array of numbers
    if (!Array.isArray(result.beat_timestamps) || result.beat_timestamps.length === 0) {
      return false;
    }

    // All beat timestamps must be valid numbers
    if (!result.beat_timestamps.every(ts => typeof ts === 'number' && isFinite(ts))) {
      return false;
    }
    ...
    const info = result.analysis_info as any;
    const requiredFields = ['total_beats', 'total_subdivisions', 'consolidated_notes', 'filtered_notes', 'min_note_duration', 'subdivision_factor'];
    
    for (const field of requiredFields) {
      if (typeof info[field] !== 'number' || !isFinite(info[field])) {
        return false;
      }
    }
  ...
  private evictLRU(): void {
    ...
    if (lruKey) {
      ...
      logger.debug("Evicted LRU cache entry", { key: lruKey.substring(0, 8) + "..." });
    }
  }
  ...
  private async findFingerprintByHash(audioHash: string): Promise<AudioFingerprint | null> {
    return await this.db.audioRepository.findByContentHash(audioHash);
  }

  private updateStats(): void {
    this.stats.totalHitCount = this.stats.l1HitCount + this.stats.l2HitCount + this.stats.l3HitCount;
    this.stats.totalMissCount = this.stats.l1MissCount + this.stats.l2MissCount + this.stats.l3MissCount;
    
    const totalRequests = this.stats.totalHitCount + this.stats.totalMissCount;
    this.stats.totalHitRate = totalRequests > 0 ? this.stats.totalHitCount / totalRequests : 0;
    
    const l1Requests = this.stats.l1HitCount + this.stats.l1MissCount;
    this.stats.l1HitRate = l1Requests > 0 ? this.stats.l1HitCount / l1Requests : 0;
    
    const l2Requests = this.stats.l2HitCount + this.stats.l2MissCount;
    this.stats.l2HitRate = l2Requests > 0 ? this.stats.l2HitCount / l2Requests : 0;
    
    const l3Requests = this.stats.l3HitCount + this.stats.l3MissCount;
    this.stats.l3HitRate = l3Requests > 0 ? this.stats.l3HitCount / l3Requests : 0;
  }

  private async emitCacheHit(audioHash: string, level: "L1" | "L2" | "DB"): Promise<void> {
    ...
    if (handlers) {
      for (const handler of handlers) {
        try {
          ...
          logger.error("Cache event handler failed", error instanceof Error ? error : new Error(String(error)), { 
